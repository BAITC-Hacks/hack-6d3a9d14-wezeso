      ...
      {requests.map(q => {
        ...
        return <article className={`request-row${isOpen ? ' is-open' : ''}`} key={q.id}>
          <Btn variant="ghost" className="request-summary" onClick={() => setExpanded(isOpen ? null : q.id)} aria-expanded={isOpen} aria-controls={`request-${q.id}`}>
            <span className="request-title"><strong>{t(event?.title || q.event_id)}</strong><span>{user.role !== 'employee' && owner ? `${owner} · ` : ''}{event ? `${t(typeName[event.type] || event.type)} · ` : ''}{date(q.session_date)}</span></span><Status status={q.status}/><CaretDown className="request-chevron" size={17}/>
          </Btn>
